...
class IntentService:
    ...
    def detect_intent(self, text: str) -> str:
        ...
        availability_patterns = [
            r'available|availability|when.*free|schedule|time slots',
    ...
    def _handle_faq(self, entities: Dict[str, Any]) -> Dict[str, Any]:
        """Handle deterministic hospital FAQ responses."""
        text_lower = entities.get("text", "").lower()
        if "visiting" in text_lower or "visitor" in text_lower or "hours" in text_lower:
            return {
                "action": "faq_answer",
                "response": "Visiting hours are typically 10 AM to 7 PM. Please confirm with the front desk for your ward.",
                "data": {"source": "static_faq", "topic": "visiting_hours"},
            }
        if "location" in text_lower or "address" in text_lower or "located" in text_lower:
            return {
                "action": "faq_answer",
                "response": "Please check the hospital website or front desk for the current address and directions.",
                "data": {"source": "static_faq", "topic": "location"},
            }
        return {
            "action": "faq_fallback",
            "response": "I can help with hospital hours, location, appointments, availability, or verified patient lookup.",
            "data": {"source": "static_faq"},
        }
